...
namespace Extractor
{
  class Program
  {
    static string BASE_ANGULAR_PATH = "..\\..\\..\\..\\..\\find-my-treasure\\";
    ...
    static void Main(string[] args)
    {
      ...
      ExtractNames(localize, realm.GameData.GetSheet<PlaceName>(), "Name", "place-name", false);
    ...
    static void ExtractConsumables(XivCollection gameData)
    {
      ...
      File.WriteAllText(AppDomain.CurrentDomain.BaseDirectory + BASE_ANGULAR_PATH + "src\\data\\sources\\foods.json", foodsJson);
      ...
      File.WriteAllText(AppDomain.CurrentDomain.BaseDirectory + BASE_ANGULAR_PATH + "src\\data\\sources\\medicines.json", medicinesJson);
    }
  ...
  static void ExtractMaps(XivCollection gameData)
  {
        
            foreach (var map in gameData.GetSheet<Map>())
            {
                try
                {
                    map.MediumImage.Save(AppDomain.CurrentDomain.BaseDirectory + BASE_ANGULAR_PATH + "src\\data\\maps\\" + map.PlaceName + ".png");
    ...
    static void ExtractActionIcons(XivCollection gameData)
    {
      ...
      File.WriteAllText(AppDomain.CurrentDomain.BaseDirectory + BASE_ANGULAR_PATH + "src\\data\\sources\\action-icons.json", json);
    }
    ...
    static void ExtractItemNames(Localize localize, ARealmReversed realm)
    {
      ...
      File.WriteAllText(AppDomain.CurrentDomain.BaseDirectory + BASE_ANGULAR_PATH + "src\\data\\assets\\items.json", json);
    }
    ...
    static void ExtractVentureNames(Localize localize, ARealmReversed realm)
    {
      ...
      string json = Regex.Replace(res.ToString(Newtonsoft.Json.Formatting.Indented), "(\"(?:[^\"\\\\]|\\\\.)*\")|\\s+", "$1");
      File.WriteAllText(AppDomain.CurrentDomain.BaseDirectory + BASE_ANGULAR_PATH + "src\\data\\sources\\ventures.json", json);
    ...
    static void ExtractNames(Localize localize, IEnumerable<IXivRow> rows, string col, string fileName, bool lazy)
    {
      ...
      if (lazy)
      {
        File.WriteAllText(AppDomain.CurrentDomain.BaseDirectory + BASE_ANGULAR_PATH + "src\\data\\assets\\" + fileName + ".json", json);
      }
      else
      {
        File.WriteAllText(AppDomain.CurrentDomain.BaseDirectory + BASE_ANGULAR_PATH + "src\\data\\assets\\" + fileName + ".json", json);
